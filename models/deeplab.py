...
class MS_Deeplab(nn.Module):
    ...
    def forward(self,x):
        ...
        x2 = self.interp1(x)
        x3 = self.interp2(x)
        out.append(F.sigmoid(self.Scale(x)))	# for original scale
        out.append(F.sigmoid(self.interp3(self.Scale(x2))))	# for 0.75x scale
        out.append(F.sigmoid(self.Scale(x3)))	# for 0.5x scale


        x2Out_interp = out[1]
        x3Out_interp = self.interp3(out[2])
        temp1 = torch.max(out[0],x2Out_interp)
        out.append(F.sigmoid(torch.max(temp1,x3Out_interp)))
        return out[0]
